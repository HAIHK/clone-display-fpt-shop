import "./Endow.scss"
import frame from "../../../assets/images/Promotion/frame.png"
import endow1 from "../../../assets/images/Promotion/endow1.jpg"
import endow2 from "../../../assets/images/Promotion/endow2.png"
import table1 from "../../../assets/images/Table/table1.jpg"
import home8 from "../../../assets/images/HomeAppliances/home8.jpg"
import home9 from "../../../assets/images/HomeAppliances/home9.jpg"
import home10 from "../../../assets/images/HomeAppliances/home10.jpg"
import home11 from "../../../assets/images/HomeAppliances/home11.jpg"

const Endow = () => {
    return (
        <div className="house-share endow">
            <div className="house-container endow">
                <div className="house-box">
                    <div className="box-img">
                        <img src={frame} alt="" className="img-child" />
                        <img src={home8} alt="" className="img-laptop" />
                    </div>
                    <div className="box-info">
                        <div className="title-name">Quạt điều hoà Kangaroo KG50F64</div>
                        <div className="price">12.490.000 Đ</div>
                    </div>
                    <div className="endow-box">
                        <div className="technology">
                            <span><i class="fa-solid fa-microchip"></i> Apple M2</span>
                            <span><i class="fa-solid fa-mobile-screen-button"></i> Chính: 11.0 inch</span>
                            <span>
                                <span><i class="fa-solid fa-clipboard"></i> 8 GB</span>
                                <span><i class="fa-solid fa-bookmark"></i> 128 GB</span>
                            </span>
                        </div>
                        <img src={endow1} alt="" className="endow-child" />
                        <img src={endow2} alt="" className="endow-child" />
                        <div className="endow-text">Giảm 200.000đ ZaloPay-QR</div>
                    </div>
                    <div className="buy-compare">
                        <div className="buy">Mua ngay</div>
                        <div className="compare">So sánh</div>
                    </div>
                </div>
                <div className="house-box">
                    <div className="box-img">
                        <img src={frame} alt="" className="img-child" />
                        <img src={home9} alt="" className="img-laptop" />
                    </div>
                    <div className="box-info">
                        <div className="title-name">Nồi cơm điện Kangaroo 1.8 lít KG18RC9</div>
                        <div className="price">12.490.000 Đ</div>
                    </div>
                    <div className="endow-box">
                        <div className="technology">
                            <span><i class="fa-solid fa-microchip"></i> Apple M2</span>
                            <span><i class="fa-solid fa-mobile-screen-button"></i> Chính: 11.0 inch</span>
                            <span>
                                <span><i class="fa-solid fa-clipboard"></i> 8 GB</span>
                                <span><i class="fa-solid fa-bookmark"></i> 128 GB</span>
                            </span>
                        </div>
                        <img src={endow1} alt="" className="endow-child" />
                        <img src={endow2} alt="" className="endow-child" />
                        <div className="endow-text">Giảm 200.000đ ZaloPay-QR</div>
                    </div>
                    <div className="buy-compare">
                        <div className="buy">Mua ngay</div>
                        <div className="compare">So sánh</div>
                    </div>
                </div>
                <div className="house-box">
                    <div className="box-img">
                        <img src={frame} alt="" className="img-child" />
                        <img src={home10} alt="" className="img-laptop" />
                    </div>
                    <div className="box-info">
                        <div className="title-name">Nồi chiên không dầu hơi nước Kalite 15</div>
                        <div className="price">12.490.000 Đ</div>
                    </div>
                    <div className="endow-box">
                        <div className="technology">
                            <span><i class="fa-solid fa-microchip"></i> Apple M2</span>
                            <span><i class="fa-solid fa-mobile-screen-button"></i> Chính: 11.0 inch</span>
                            <span>
                                <span><i class="fa-solid fa-clipboard"></i> 8 GB</span>
                                <span><i class="fa-solid fa-bookmark"></i> 128 GB</span>
                            </span>
                        </div>
                        <img src={endow1} alt="" className="endow-child" />
                        <img src={endow2} alt="" className="endow-child" />
                        <div className="endow-text">Giảm 200.000đ ZaloPay-QR</div>
                    </div>
                    <div className="buy-compare">
                        <div className="buy">Mua ngay</div>
                        <div className="compare">So sánh</div>
                    </div>
                </div>
                <div className="house-box">
                    <div className="box-img">
                        <img src={frame} alt="" className="img-child" />
                        <img src={home11} alt="" className="img-laptop" />
                    </div>
                    <div className="box-info">
                        <div className="title-name">Robot hút bụi Takdir V6S VC</div>
                        <div className="price">12.490.000 Đ</div>
                    </div>
                    <div className="endow-box">
                        <div className="technology">
                            <span><i class="fa-solid fa-microchip"></i> Apple M2</span>
                            <span><i class="fa-solid fa-mobile-screen-button"></i> Chính: 11.0 inch</span>
                            <span>
                                <span><i class="fa-solid fa-clipboard"></i> 8 GB</span>
                                <span><i class="fa-solid fa-bookmark"></i> 128 GB</span>
                            </span>
                        </div>
                        <img src={endow1} alt="" className="endow-child" />
                        <img src={endow2} alt="" className="endow-child" />
                        <div className="endow-text">Giảm 200.000đ ZaloPay-QR</div>
                    </div>
                    <div className="buy-compare">
                        <div className="buy">Mua ngay</div>
                        <div className="compare">So sánh</div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Endow